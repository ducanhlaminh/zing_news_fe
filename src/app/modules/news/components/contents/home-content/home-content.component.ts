...
export class HomeContentComponent {
    ...
    // tin moi
    getNewArtcles() {
        this.NewsService.getNewArtclesMain(this.page).subscribe((data: any) => {
            this.newArtcles = [...this.newArtcles, ...data.newArticleCate.rows];
        });
    }
    onScrollDown() {
        if (this.page < 10) {
    ...
    getHotNews() {
        this.NewsService.getHotMain().subscribe((data: any) => {
            ...
            this.artclesHotMain = {
                left: [result[3], result[4], result[5], result[6], result[7]],
